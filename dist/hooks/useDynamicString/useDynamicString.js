...
export const useDynamicString = (input) => {
    const { isDynamic, datasource, loading, strapiLocalization } = useDynamicApi();
    const theme = useThemeConfig();
    const [, i18n] = useTranslation();
    const [userLocale] = useLocalStorage('userLocale');
    const locale = userLocale || i18n.language;
    ...
    return useMemo(() => {
        ...
        const replacements = Array.from((input ?? '').matchAll(new RegExp(/\{(.*?)\}/g)));
        let text = input ?? '';
        let loaded = true;
        replacements.forEach((item) => {
            ...
            if (loaded && !hasFirstLoad)
                loaded = false;
            if (strapiLocalization) {
                const localeKey = modifyStringPath(key, `localizations.${locale.split('-')?.[0]}`);
                const value = _.get(datasource, localeKey, '');
                if (value)
                    text = text.replace(q, value);
                else {
                    let newValue = '';
                    i18nLocales?.some((val) => {
                        const modifiedKey = modifyStringPath(key, `localizations.${val?.code?.split('-')?.[0]}`);
...
export const getDynamicString = (input, data) => {
    const replacements = Array?.from((input ?? '')?.matchAll(new RegExp(/\{(.*?)\}/g)));
    let text = input ?? '';
    let loaded = true;
    replacements?.forEach((item) => {
...
export const getI18nString = (input, locale, theme) => {
    const i18nJson = theme?.defaultTheme?.configurations?.contentData?.i18nJson?.values;
    const i18nLocales = theme?.defaultTheme?.configurations?.contentData?.i18nJson
        ?.locales;
    if (input?.includes('&&') || input?.includes('&amp;&amp;')) {
        const inputToUse = input?.includes('&amp;&amp;')
            ? unescapeHtml(input)
            : input;
        const findItem = i18nJson?.find((res) => res.jsonString === inputToUse);
        if (findItem) {
            ...
            const baseLanguageMatch = i18nLocales.find((val) => val?.code === baseLanguage);
